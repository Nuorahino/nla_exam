...
TEMPLATE_TEST_CASE("Householder Vector is unit vector, if the last n-1 entries are 0",
  ...
  REQUIRE_THAT(w(Eigen::seqN(1, n - 1)).norm(),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
  REQUIRE_THAT(std::abs(w(0) - TestType{1.0}),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}
...
TEMPLATE_TEST_CASE("Computed Householder Vector combined with LAPACK introduces zeros",
  ...
  REQUIRE_THAT(res(Eigen::lastN(n - 1)).norm(),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}


TEMPLATE_TEST_CASE(
    "Computed Householder Vector combined with apply Householder left introduces zeros",
  ...
  REQUIRE_THAT(v(Eigen::lastN(n - 1)).norm(),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}


TEMPLATE_TEST_CASE(
    "Computed Householder Vector combined with apply Householder right introduces zeros",
    "[householder][compute_householder][apply_householder]", float, double,
    std::complex<float>, std::complex<double>) {
  int n = GENERATE(take(5, random<int>(3, 10)));
  Eigen::Vector<TestType, -1> v = Eigen::Vector<TestType, -1>::Random(n);
...
TEMPLATE_TEST_CASE(
    "LAPCAK Householder Vector with Apply left Householder introduces Zeros",
    "[householder][compute_householder]", float, double, std::complex<float>,
    std::complex<double>) {
  int n = GENERATE(take(5, random<int>(3, 10)));
  Eigen::Vector<TestType, -1> v = Eigen::Vector<TestType, -1>::Random(n);
...
TEMPLATE_TEST_CASE(
    "LAPCAK Householder Vector with Apply right Householder introduces Zeros",
    "[householder][compute_householder]", float, double, std::complex<float>,
    std::complex<double>) {
  int n = GENERATE(take(5, random<int>(3, 10)));
  Eigen::Vector<TestType, -1> v = Eigen::Vector<TestType, -1>::Random(n);
...
TEMPLATE_TEST_CASE(
  ...
  REQUIRE_THAT(A(Eigen::lastN(n - 1), i).norm(),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}
...
TEMPLATE_TEST_CASE(
  ...
  nla_exam::ApplyHouseholderRight(w.conjugate(), A);
  REQUIRE_THAT(A(i, Eigen::lastN(n - 1)).norm(),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}
...
//   REQUIRE_THAT((w_n(Eigen::lastN(n-1)) - w).norm(), Catch::Matchers::WithinAbs(0.0,
//   tol<TestType>()));
// }
// w_n = w_n / w_n(0);
// TestType w0 = w_n(0);
...
//   //REQUIRE_THAT(w(Eigen::seqN(1, n-1)).norm(), Catch::Matchers::WithinAbs(0.0,
//   tol<TestType>()));
// }
