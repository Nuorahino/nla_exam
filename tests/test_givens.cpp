...
TEMPLATE_TEST_CASE("Arithmetic Givens Parameter for first parameter = 0 is correct",
...
TEMPLATE_TEST_CASE("Arithmetic Givens Parameter for second parameter = 0 is correct",
                   "[givens][givens_parameter]", float, double, std::complex<float>,
                   std::complex<double>) {
  TestType a = GENERATE(take(5, filter([](TestType i) { return i != TestType{0}; },
                                       ComplexRandom<TestType>(-100, 100))));
  TestType b = 0;
...
TEMPLATE_TEST_CASE("Givens Vector is of length one", "[givens][givens_parameter]", float,
                   double, std::complex<float>, std::complex<double>) {
  TestType a = GENERATE(take(5, ComplexRandom<TestType>(-100, 100)));
  TestType b = GENERATE(take(5, ComplexRandom<TestType>(-100, 100)));
  auto res = nla_exam::GetGivensEntries(a, b);
  REQUIRE_THAT(std::hypot(std::abs(res.at(0)), std::abs(res.at(1))),
               Catch::Matchers::WithinAbs(1, tol<TestType>()));
}


TEMPLATE_TEST_CASE("Compare Givens Parameter with LAPACK", "[givens][givens_parameter]",
  ...
  REQUIRE_THAT(std::abs(res_qr.at(0) - res_lapack.at(0)),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
  REQUIRE_THAT(std::abs(res_qr.at(1) - res_lapack.at(1)),
               Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}


TEMPLATE_TEST_CASE("Right Givens Rotation does not change the Norm", "[givens]", float,
                   double, std::complex<float>, std::complex<double>) {
  constexpr int n = 10;
  ...
  nla_exam::ApplyGivensRight(A(Eigen::all, {i, j}), param.at(0), param.at(1),
                             param.at(2));
  REQUIRE_THAT(A.norm(),
...
TEMPLATE_TEST_CASE("Left Givens Rotation does not change the Norm", "[givens]", float,
                   double, std::complex<float>, std::complex<double>) {
  constexpr int n = 10;
  int i = GENERATE(take(5, random<int>(0, n - 1)));
  int j = GENERATE(take(5, random<int>(0, n - 1)));
...
TEMPLATE_TEST_CASE(
    "Givens Parameter combined with applying the left givens rotation using LAPACK sets "
    "the entry to 0",
    "[givens][givens_parameter]", float, double, std::complex<float>,
    std::complex<double>) {
  ...
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  std::vector<TestType> param = nla_exam::GetGivensEntries(A(0, 0), A(1, 0));
  Eigen::Matrix<TestType, -1, -1> res =
      apply_givens_left<TestType>(A, 0, get_real(param.at(0)), param.at(1));
  REQUIRE_THAT(std::abs(res(1, 0)), Catch::Matchers::WithinAbs(0.0, tol<TestType>()));
}


TEMPLATE_TEST_CASE(
    "Givens Parameter combined with applying the right givens rotation using LAPACK sets "
    "the entry to 0",
    "[givens][givens_parameter]", float, double, std::complex<float>,
    std::complex<double>) {
  int n = GENERATE(take(5, random<int>(3, 10)));
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  std::vector<TestType> param = nla_exam::GetGivensEntries(A(0, 0), A(0, 1));
  Eigen::Matrix<TestType, -1, -1> res = apply_givens_right<TestType>(
      A, 0, get_real(param.at(0)), complex_conj(param.at(1)));
...
TEMPLATE_TEST_CASE("Left Givens Rotation has same result as LAPACK",
  ...
  std::vector<TestType> param = nla_exam::GetGivensEntries(A(0, 0), A(1, 0));
  Eigen::Matrix<TestType, -1, -1> res =
      apply_givens_left<TestType>(A, i, j, get_real(param.at(0)), param.at(1));
  nla_exam::ApplyGivensLeft(A({i, j}, Eigen::all), param.at(0), param.at(1), param.at(2));
...
TEMPLATE_TEST_CASE(
    "Real Right Givens Rotation has same result in real and complex function",
    "[givens][apply_givens]", float, double) {
  constexpr int n = 20;
  int i = GENERATE(take(5, random<int>(0, n - 1)));
  int j = GENERATE(take(5, random<int>(0, n - 1)));
  if (i == j) j = (i + 1) % (n - 1);
...
TEMPLATE_TEST_CASE(
    "Real Left Givens Rotation has same result in real and complex function",
    "[givens][apply_givens]", float, double) {
  constexpr int n = 20;
  int i = GENERATE(take(5, random<int>(0, n - 1)));
  int j = GENERATE(take(5, random<int>(0, n - 1)));
...
TEMPLATE_TEST_CASE("Right Givens Rotation does not change other columns",
                   "[givens][apply_givens]", float, double, std::complex<float>,
                   std::complex<double>) {
  constexpr int n = 20;
  int i = GENERATE(take(5, random<int>(0, n - 1)));
...
TEMPLATE_TEST_CASE("Left Givens Rotation does not change other columns",
                   "[givens][apply_givens]", float, double, std::complex<float>,
                   std::complex<double>) {
  constexpr int n = 20;
  int i = GENERATE(take(5, random<int>(0, n - 1)));
