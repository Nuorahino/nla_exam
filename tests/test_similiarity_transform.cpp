...
TEMPLATE_TEST_CASE("Non Symmetric Hessenberg Transformation has the desired form",
    "[HessenbergTransformation]", float, double, std::complex<float>, std::complex<double>) {
...
TEMPLATE_TEST_CASE("Non Symmetric Hessenberg Transformation does not change the eigenvalues",
    "[HessenbergTransformation]", float, double, std::complex<float>, std::complex<double>) {
  ...
  nla_exam::HessenbergTransformation(A);
  std::tie(eigenvector, new_eigenvalues) = CalculateGeneralEigenvalues<TestType>(A, false);
  std::vector<typename ComplexDataType<TestType>::type> ev(eigenvalues.data(), eigenvalues.data() + n);
  std::vector<typename ComplexDataType<TestType>::type> nev(new_eigenvalues.data(), new_eigenvalues.data() + n);
  min_matching(ev, nev);
  for (unsigned int i = 0; i < ev.size(); ++i) {
    REQUIRE_THAT(std::abs(ev.at(i) - nev.at(i)), Catch::Matchers::WithinAbs(0.0, tol<TestType>() * 1e2));
  }
}
...
TEMPLATE_TEST_CASE("Non Symmetric Givens Rotation does not change the eigenvalues",
  ...
  std::tie(eigenvector, eigenvalues) = CalculateGeneralEigenvalues<TestType>(A, false);
  nla_exam::ApplyGivensRight(A, givens_param.at(0), givens_param.at(1), givens_param.at(2));
  nla_exam::ApplyGivensLeft(A, givens_param.at(0), givens_param.at(1), givens_param.at(2));
  std::tie(eigenvector, new_eigenvalues) = CalculateGeneralEigenvalues<TestType>(A, false);
  std::vector<typename ComplexDataType<TestType>::type> ev(eigenvalues.data(), eigenvalues.data() + n);
  std::vector<typename ComplexDataType<TestType>::type> nev(new_eigenvalues.data(), new_eigenvalues.data() + n);
  min_matching(ev, nev);
...
TEMPLATE_TEST_CASE("Implicit QR Step does not change the eigenvalues",
    "[HessenbergTransformation]", std::complex<float>, std::complex<double>) {
  int n = GENERATE(take(5, random<int>(3, 40)));
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  nla_exam::HessenbergTransformation(A);
...
TEMPLATE_TEST_CASE("Symmetric Implicit QR step does not change the eigenvalues",
    "[HessenbergTransformation]", float, double) {
  int n = GENERATE(take(5, random<int>(3, 40)));
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  A = A + A.adjoint().eval();
  nla_exam::HessenbergTransformation(A, true);
...
TEMPLATE_TEST_CASE("Implicit QR Step with exceptional shift does not change the eigenvalues",
    "[HessenbergTransformation]", float, double, std::complex<float>, std::complex<double>) {
  int n = GENERATE(take(5, random<int>(3, 40)));
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  nla_exam::HessenbergTransformation(A);
...
TEMPLATE_TEST_CASE("Symmetric Implicit QR step with exceptional shift does not change the eigenvalues",
    "[HessenbergTransformation]", float, double) {
  int n = GENERATE(take(5, random<int>(3, 40)));
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  A = A + A.adjoint().eval();
...
TEMPLATE_TEST_CASE("Double Shift QR Step with exceptional shift does not change the eigenvalues",
    "[HessenbergTransformation]", float, double) {
  int n = GENERATE(take(5, random<int>(3, 40)));
  Eigen::Matrix<TestType, -1, -1> A = Eigen::Matrix<TestType, -1, -1>::Random(n, n);
  nla_exam::HessenbergTransformation(A);
