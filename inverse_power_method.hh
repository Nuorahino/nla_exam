...
template<class DerivedMat, class DerivedVec,
  typename T = typename DerivedVec::Scalar>
T InversePowerMethod(const Eigen::MatrixBase<DerivedMat>& ak_a,
    const typename DerivedMat::Scalar ak_shift,
    const Eigen::MatrixBase<DerivedVec>& a_v,
    const double ak_tol) {
  typedef Eigen::Matrix<typename DerivedMat::Scalar, -1, -1> MatrixType;
  MatrixType shifted_a = ak_a - MatrixType::Identity(ak_a.rows(), ak_a.cols())
    * 4;
