...
Eigen::Vector<T, -1> GetHouseholderVector(
    const Eigen::MatrixBase<Derived> &ak_x) {
  Eigen::Vector<T, -1> w = ak_x;
  long n = w.rows();
  T t = w(Eigen::lastN(n - 1)).squaredNorm();
  if (std::abs(t) < std::numeric_limits<decltype(std::abs(
                        t))>::min()) {  // Better Criteria needed
    w(0) = 1;
  } else {
    T s = std::sqrt(std::abs(w(0)) * std::abs(w(0)) + t);
    if constexpr (IsComplex<typename Derived::Scalar>()) {
      s *= std::polar(1.0,
                      std::arg(w(0)));  // Choise to avoid loss of significance
    } else {
      if (w(0) < 0) s *= -1;
    }
    w(0) = w(0) + s;
...
void ApplyHouseholderRight(const Eigen::MatrixBase<Derived2> &ak_w,
  ...
  MatrixType &matrix =
      const_cast<MatrixType &>(a_matrix);  // Const cast needed for eigen
...
void ApplyHouseholderLeft(const Eigen::MatrixBase<Derived2> &ak_w,
  ...
  for (int i = 0; i < a_matrix.cols(); ++i) {
    T tmp =
        beta * ak_w.dot(a_matrix(Eigen::all, i));  // w.dot(A) = w.adjoint() * A
...
void HessenbergTransformation(const Eigen::MatrixBase<Derived> &a_matrix,
  ...
  for (int i = 0; i < matrix.rows() - 2; ++i) {
    ...
    ApplyHouseholderLeft(w,
                         matrix(Eigen::lastN(n - i - 1), Eigen::seq(i, n - 1)));
...
inline std::enable_if_t<is_symmetric && std::is_arithmetic<DataType>::value,
                        DataType>
...
WilkinsonShift(const Eigen::MatrixBase<Derived> &ak_matrix) {
  DataType d = (ak_matrix(0, 0) - ak_matrix(1, 1)) / 2.0;
  // TODO: Find a way to compute this which avoids over and underflow
  if (d >= 0) {
    return ak_matrix(1, 1) + d -
           std::sqrt(d * d + ak_matrix(1, 0) * ak_matrix(0, 1));
  } else {
    return ak_matrix(1, 1) + d +
           std::sqrt(d * d + ak_matrix(1, 0) * ak_matrix(0, 1));
  }
...
template <class DataType, bool is_symmetric, class Derived>
std::enable_if_t<IsComplex<DataType>(), DataType> WilkinsonShift(
    const Eigen::MatrixBase<Derived> &ak_matrix) {
  ...
  DataType entry;
  entry = ak_matrix(1, 1);
  if (std::abs(ev1 - entry) <
      std::abs(ev2 - entry)) {  // return the nearest eigenvalue
    return ev1;
  } else {
    return ev2;
  }
...
std::enable_if_t<std::is_arithmetic<DataType>::value, void> ApplyGivensLeft(
    const Eigen::MatrixBase<Derived> &a_matrix, const DataType ak_c,
    const DataType ak_s) {
 ...
 */
template <class DataType, class Derived>
std::enable_if_t<std::is_arithmetic<DataType>::value, void> ApplyGivensRight(
    const Eigen::MatrixBase<Derived> &a_matrix, const DataType ak_c,
    const DataType ak_s) {
  Eigen::MatrixBase<Derived> &matrix =
      const_cast<Eigen::MatrixBase<Derived> &>(a_matrix);
 ...
 */
template <class DataType, class Derived>
void ApplyGivensLeft(const Eigen::MatrixBase<Derived> &a_matrix,
 ...
 */
template <class DataType, class Derived>
void ApplyGivensRight(const Eigen::MatrixBase<Derived> &a_matrix,
                      const DataType ak_c, const DataType ak_s,
                      const DataType ak_sconj) {
  Eigen::MatrixBase<Derived> &matrix =
      const_cast<Eigen::MatrixBase<Derived> &>(a_matrix);
...
inline std::enable_if_t<std::is_arithmetic<DataType>::value,
                        std::vector<DataType>>
...
ExceptionalSingleShift() {
  ...
  std::uniform_real_distribution<DataType> dist(
      -100, 100);  // distribution in range [-100, 100]
  return dist(rng);
}
...
ExceptionalSingleShift() {
  std::random_device dev;
  std::mt19937 rng(dev());
  std::uniform_real_distribution<typename DataType::value_type> dist(
      -100, 100);  // distribution in range [-100, 100]
...
void ImplicitQrStep(const Eigen::MatrixBase<Derived> &a_matrix,
  ...
  auto entries =
      GetGivensEntries<>(a_matrix(0, 0) - shift,
                         a_matrix(1, 0));  // Parameter for the initial step
  if constexpr (is_symmetric) {  // TODO maybe better solution if matrix has
                                 // symmetric view
    // innitial step
    switch (n) {
      case 2:
    ...
    for (int k = 1; k < n - 2; ++k) {
      ...
      ApplyGivensLeft<DataType>(
          matrix(Eigen::seqN(k, 2), Eigen::seq(k - 1, n - 1)), entries.at(0),
          entries.at(1));
      ApplyGivensRight<DataType>(
          matrix(Eigen::seq(0, k + 2), Eigen::seqN(k, 2)), entries.at(0),
          entries.at(1));
      matrix(k - 1, k + 1) = 0;
      matrix(k + 1, k - 1) = 0;
    }
    entries =
        GetGivensEntries<>(a_matrix(n - 2, n - 3), a_matrix(n - 1, n - 3));
...
#ifdef IMPLICIT
    return std::vector<typename Derived::Scalar>{};  // For real eigenvalues do
                                                     // a single step
#endif
...
void DoubleShiftQrStep(const Eigen::MatrixBase<Derived> &a_matrix,
  ...
  Matrix m1 =
      a_matrix(Eigen::seqN(0, 3), Eigen::all) * a_matrix(Eigen::all, 0) +
  ...
  Eigen::Vector<typename Derived::Scalar, -1> w =
      GetHouseholderVector(m1);  //  initial step
  ...
  for (int i = 0; i < n - 3; ++i) {  //  Buldge chasing
    ...
    ApplyHouseholderLeft(w,
                         matrix(Eigen::seqN(i + 1, 3), Eigen::seq(i, n - 1)));
    matrix(Eigen::seqN(i + 2, 2), i) =
        Matrix::Zero(2, 1);  // Set Round off errors to 0
  }
...
int DeflateDiagonal(const Eigen::MatrixBase<Derived> &a_matrix, int &a_begin,
  ...
  for (int i = a_end; i > a_begin; --i) {
    if (std::abs(a_matrix(i, i - 1)) <=
        (ak_tol *
         (std::abs(a_matrix(i, i)) + std::abs(a_matrix(i - 1, i - 1))))) {
 ...
 */
template <class DataType, class Derived>
std::vector<DataType> CalcEigenvaluesFromSchur(
    const Eigen::MatrixBase<Derived> &ak_matrix,
...
#pragma GCC diagnostic pop
        ...
        Eigen::MatrixXcd test =
            ak_matrix(Eigen::seq(i, i + 1), Eigen::seq(i, i + 1));
...
#pragma GCC diagnostic ignored "-Wfloat-equal"
#pragma GCC diagnostic push
    if (std::abs(ak_matrix(ak_matrix.rows() - 1, ak_matrix.rows() - 2)) ==
        0.0)  // Last EV is in a diagonal block
#pragma GCC diagnostic pop
      res.at(ak_matrix.rows() - 1) =
          ak_matrix(ak_matrix.rows() - 1,
...
std::vector<DataType> QrIterationHessenberg(
    const Eigen::MatrixBase<Derived> &a_matrix, const double ak_tol = 1e-12) {
  ...
  while (end_of_while < end) {
    ...
    if (status > 0) {
      ...
      if (steps_since_deflation > 10) {
        ...
        //         if ((std::abs(a_matrix(Eigen::seq(begin, end),
        //         Eigen::seq(begin, end)).trace() - a_matrix(begin, begin) *
        //         DataType{end - begin + 1}) + a_matrix(Eigen::seq(begin, end),
        //         Eigen::seq(begin, end)).diagonal(-1).norm()) < ak_tol) {
        // #pragma GCC diagnostic pop
        //           const_cast<MatrixType&>(a_matrix)(Eigen::seq(begin,
        //           end),Eigen::seq(begin, end)).diagonal(-1).setZero(); end =
        //           begin; begin = 0; continue;
        //         }
 ...
 */
template <bool IsHermitian, typename Derived,
          class DataType =
              typename DoubleType<IsComplex<typename Derived::Scalar>()>::type,
          class ComplexDataType =
              typename EvType<IsComplex<DataType>(), DataType>::type>
std::vector<ComplexDataType> QrMethod(
    const Eigen::MatrixBase<Derived> &ak_matrix, const double ak_tol = 1e-12) {
